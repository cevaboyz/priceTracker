...
mediaworld_xml_feed <- "https://www.mediaworld.it/sitemaps/sitemap-productdetailspages-000.xml"


mediaworld_rss_xml <- as_list(read_xml(mediaworld_xml_feed))



xml_df <- tibble::as_tibble(mediaworld_rss_xml) %>%
  unnest_longer(urlset)
...
lp_df <- as.data.frame(lp_df)

lp_df_unique <- unique(lp_df)

lp_df_unique <- lp_df_unique$urlset

lp_df_unique_df <- as.data.frame(lp_df_unique)

lp_df_unique_2_imetec <- as.data.frame(lp_df_unique_df[lp_df_unique_df$lp_df_unique  %like% "imetec",])

lp_df_unique_2_bellissima <- as.data.frame(lp_df_unique_df[lp_df_unique_df$lp_df_unique  %like% "bellissima",])


media_imetec_url <- unlist(lp_df_unique_2_imetec$`lp_df_unique_df[lp_df_unique_df$lp_df_unique %like% "imetec", ]`)

media_bellissima_url <- unlist(lp_df_unique_2_bellissima$`lp_df_unique_df[lp_df_unique_df$lp_df_unique %like% "bellissima", ]`)

url_media <- c(media_imetec_url,media_bellissima_url)
...
mediaworld_price_checker <- function(url_list) {
  
  ua <- sample(user_agent_list,1,replace=FALSE)
  ...
  json_data <- scripts <-
    scrape(url, content = "text/html; charset=UTF-8") %>% html_nodes("script") %>%  .[1] %>% html_text()
  ...
  brand <- ex_parsed_json$brand$name
  
  product_sku <- ex_parsed_json$sku
  
  product_name <- ex_parsed_json$name
  
  current_price <- ex_parsed_json$offers$price
  
  availability <- ex_parsed_json$offers$availability
  
  availability_2 <- strsplit(availability, "/+")
  
  availability <- availability_2[[1]]
  
  availability <- availability[3]
  
  url_correct <- ex_parsed_json$url
...
media_raw_data <- purrr::map(url_media, purrr::possibly(mediaworld_price_checker, NA))

media_raw_data_df <- plyr::ldply(media_raw_data, data.frame)
